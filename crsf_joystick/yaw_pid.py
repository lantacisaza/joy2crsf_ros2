...
class PID_control(Node):
   def __init__(self):
       ...
       self.pid_pitch = PID(kp=60, ki=2.0, kd=25.0)
       self.pid_roll = PID(kp=75, ki=1.0, kd=15.0)
       self.pid_alt = PID(kp=10, ki=0.0, kd=5.0)
   ...
   def joy_callback(self, msg: Joy):
       ...
       if self.arm_state == 2000:
           if not self.armed_flag:
               ...
               print("Armed, holding throttle min")
           else:
               goal_x = 0.0  # forward/back
               goal_y = 0.6  # altitude (up/down)
               goal_z = 0.0  # left/right
               goal_yaw = 0.0

               #YAW PID
               yaw_error = goal_yaw - self.yaw_actual
               yaw_correction = self.pid_yaw.compute(yaw_error, dt)
               yaw = pwm_to_crsf(int( 1500))
               ...
               pitch_correction = self.pid_pitch.compute(pitch_error, dt)
               roll_correction = self.pid_roll.compute(roll_error, dt)
               pitch = pwm_to_crsf(int(pitch_correction+1500))
               roll = pwm_to_crsf(int(roll_correction+1500))
               #pitch = pwm_to_crsf(int(1500))




               cos_pitch=math.cos(pitch_correction)
               cos_roll=math.cos(roll_correction)
               cos_compensation=max(0.7, cos_pitch * cos_roll)
               velocity_actual = (self.y_actual - self.y_prev) / dt
               ...
               throttle = pwm_to_crsf(int(min(max(throttle_raw, 1000), 2000)))

               self.y_prev = self.y_actual
               print("--------------------------------------------------------------------------")
               print("roll",roll_correction,roll_error,roll_correction+1500,self.z_actual)
               print("pitch", pitch_correction, pitch_error, pitch_correction + 1500, self.x_actual)
               print("yaw", yaw_correction, yaw_error, yaw_correction + 1500)
               print("throttle", throttle_correction, throttle_raw, self.y_actual)
               print("--------------------------------------------------------------------------")
               with open(self.traj_file, mode='a', newline='') as f:
                   writer = csv.writer(f)
                   writer.writerow([self.x_actual, self.y_actual, self.z_actual,roll_correction+1500,pitch_correction+1500,throttle_raw, yaw_correction+1500])
       else:
           throttle = 1000
           self.armed_flag = False
           self.pid_yaw.integral = self.pid_pitch.integral = self.pid_roll.integral = self.pid_alt.integral = 0
           self.pid_yaw.prev_error = self.pid_pitch.prev_error = self.pid_roll.prev_error = self.pid_alt.prev_error = 0


       aux = [pwm_to_crsf(1000)] * 10
       channels = [roll, pitch,throttle, yaw, pwm_to_crsf(self.arm_state), pwm_to_crsf(1500)] + aux
