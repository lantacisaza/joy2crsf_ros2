   ...
   def joy_callback(self, msg: Joy):
       ...
       if self.arm_state == 2000:
           if not self.armed_flag:
               ...
               error = goal_yaw - self.yaw_actual

               # Update integral and derivative
               self.integral += error * dt
               derivative = (error - self.latest_error) / dt
               ...
               if error <= 0.04 and error > -0.04:
                   ...
                   yaw = pwm_to_crsf(int(pid_output + 1500))

               print(
                   f"error: {error:.4f}, yaw: {self.yaw_actual:.4f}, pwm: {int(pid_output + 1500)}")
