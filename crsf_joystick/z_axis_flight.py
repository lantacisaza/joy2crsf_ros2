...
class JoyToCRSF(Node):
    def __init__(self):
        ...
        self.y_prev = 0.0  # or some reasonable default

        self.csv_file = "flight_log.csv"
        # If file doesn't exist, create it with headers
        if not os.path.isfile(self.csv_file):
            with open(self.csv_file, mode='w', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(["actual_height", "position_error", "velocity_actual", "velocity_goal", "throttle"])
    ...
    def joy_callback(self, msg):
        ...
        if current_button_4 == 1 and self.prev_button_4 == 0:
            ...
            print(f"Toggled arm: {self.arm_state}", f"Toggled angle: {self.angle_state}")
    ...
    def control_loop(self):
        ...
        throttle = 1000  # Default low throttle
        if self.arm_state == 2000:
            if not self.armed_flag:
                ...
                kv = 50
                y_goal = 0.3
                dt = 1/150 #150hz. CHange as needed and seen from TX display
                position_error = y_goal - self.y_actual
                velocity_actual = (self.y_actual - self.y_prev) / dt
                velocity_goal = kp * (y_goal - self.y_actual)

                throttle = hover_throttle + kv * (velocity_goal - velocity_actual)
                ...
                with open(self.csv_file, mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([self.y_actual, position_error, velocity_actual, velocity_goal, throttle])

                # Print as before
                print("actual height", self.y_actual,
                      "position error", position_error,
                      "velocity actual", velocity_actual,
                      "velocity goal", velocity_goal,
                      "throttle:", throttle)
        ...
        channels = [roll, pitch, pwm_to_crsf(throttle), yaw, pwm_to_crsf(self.arm_state), pwm_to_crsf(self.angle_state)] + aux

        frame = build_frame(channels)
        self.ser.write(frame)
        #print("Sent frame:", " ".join(f"{b:02x}" for b in frame))
